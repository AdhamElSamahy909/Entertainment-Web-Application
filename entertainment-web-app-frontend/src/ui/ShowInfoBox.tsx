...
function ShowInfoBox({
  ...
  return (
    <div className={`flex flex-col gap-[4px] font-medium text-${color}`}>
      <div className="flex text-[0.75rem] items-center gap-[0.5rem] h-[1rem] font-light opacity-75">
        <p className=" ">{show.year}</p>
        <span
          className={`w-[3px] h-[3px] opacity-50 bg-${color} rounded-full`}
        ></span>
        <div className="flex justify-center sm:text-[0.9379rem] items-center gap-[0.375rem]">
          {show.category === "Movie" ? (
            <svg
              ...
              className={`${trending === true ? "fill-gray-300" : color === "white" ? "fill-white" : "fill-secondary"}`}
            >
              <path
